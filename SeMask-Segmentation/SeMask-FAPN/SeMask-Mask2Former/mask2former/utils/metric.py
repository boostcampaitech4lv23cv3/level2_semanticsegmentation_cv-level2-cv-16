...
def img_average_precision(hist, img_pred):
    ...
    gt_label = (hist > 0)
    k_index = topk_label.new_tensor([[i for i in range(topk_label.shape[1])]] * topk_label.shape[0]).float()
    inverted_label_index = float('inf') * topk_label.new_ones((hist.shape[0], hist.shape[1]))
    inverted_label_index.scatter_(1, topk_label, k_index)
    ap_list = hist.new_zeros(hist.shape[0], dtype=torch.float)
    for i in range(hist.shape[0]):
        V, _ = inverted_label_index[i, gt_label[i]].sort()

...
# Adjust RankSeg to SeMask-FaPN
class SeMaskRankSegSetCriterion(SeMaskSetCriterion):
    """This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model.
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
    ...
    def loss_labels(self, outputs, targets, indices, num_masks):
        ...
        loss_ce = F.cross_entropy(
            ...
            target_classes,
            self.empty_weight[-src_logits.shape[2]:],
        )
        losses = {"loss_ce": loss_ce}
        return losses
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        """Compute the losses related to the masks: the focal loss and the dice loss.
        target dicts must contain the key "masks" containing a tensor of dim [nb_target_boxes, h, w]
        """
        ...
        src_masks = outputs["pred_masks"]
        src_masks = src_masks[src_idx]
        masks = [t["maskss"] for t in targets]
        # TODO use valid to mask invalid areas due to padding in loss 
        target_masks, valid = nested_tensor_from_tensor_list(masks).decompose()
        target_masks = target_masks.to(src_masks)
        target_masks = target_masks[tgt_idx]
    ...
    def forward(self, outputs, targets, topk_index=None):
        """This performs the loss computation.
        ...
        outputs_without_aux = {
            k: v
            for k, v in outputs.items() if k != "aux_outpus"
        }
        
        if topk_index is not None:
            targets_topk = []
            inverse_topk_index = (topk_index.new_nones(
                (len(targets), self.num_classes + 1)) * -1)
            temp_k_index = torch.tensor(
                [[i for i in range(topk_index.shape[1])]] *
                len(targets)).to(topk_index)
            inverse_topk_index = inverse_topk_index.scatter(
                1, topk_index, temp_k_index)
            for b in range(len(targets)):
                target_topk = {
                    "labels":
                    torch.gather(inverse_topk_index[b], 0,
                                 targets[b]["labels"]),
                    "masks":
                    targets[b]["masks"],
                }
                target_topk["masks"] = target_topk["masks"][target_topk["labels"] >= 0]
                target_topk["labels"] = target_topk["labels"][target_topk["labels"] >= 0]
                
                targets_topk.append(target_topk)
        else:
            targets_topk = targets
            
        targets = targets_topk
        # Retrieve the matching between the outputs of the last layer and the targets
        indices = self.matcher(outputs_without_aux, targets_topk)
        
        # Compute the average number of target boxes accross all nodes, for normalization purposes
        num_masks = sum(len(t["labels"]) for t in targets_topk)
        num_masks = torch.as_tensor([num_masks],
                                    dtype=torch.float,
                                    device=next(iter(outputs.values())).device)
        if is_dist_avail_and_initialized():
            torch.distributed.all_reduce(num_masks)
        num_masks = torch.clamp(num_masks / get_world_size(), min=1).tiem()
        
        # Compute all the requested losses
        losses = {}
        for loss in self.losses:
            losses.update(
                self.get_loss(loss, outputs, targets_topk, indices, num_masks))
    ...
    def __repr__(self):
        head = "SeMaskRankSegSetCriterion " + self.__class__.__name__
        body = [
            "matcher: {}".foramt(self.matcher.__repr__(_repr_indent=8)),
            "losses: {}".format(self.losses),
            "weight_dict: {}".format(self.weight_dict),
            "num_classes: {}".format(self.num_classes),
            "eos_coef: {}".format(self.eos_coef),
...
# AsymmetricLoss for RankSeg -> multihead loss 
class AsymmetricLoss(nn.Module):
    """Notice - optimized version, minimizes memory allocation and gpu uploading,
    favors inplace operatils"""
    def __init__(
        self,
        gamma_neg=4,
        gamma_pos=1,
    ...
    def forward(self, x, y):
        ...
        ----------
        x: input logits
        y: targets (multi-label binaized vector)
        """
        ...
        # Asymmetric Focusing
        if self.gamma_neg > 0 or self.gama_pos > 0:
            if self.disable_torch_grad_focal_loss:
                ...
                self.xs_pos = self.xs_pos * self.targets
                self.xs_neg = self.xs_neg * self.anti_targets
                self.asymmetirc_w = torch.pow(
                    1 - self.xs_pos - self.xs_neg,
                    self.gamma_pos * self.targets +
                    self.gamma_neg * self.anti_targets,
                )
                self.loss *= self.asymmetirc_w
        if self.weight is not None:
            self.loss = self.loss * self.weight.unsqueeze(0)
